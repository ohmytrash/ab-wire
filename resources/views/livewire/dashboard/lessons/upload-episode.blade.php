...
>
  ...
  <div class="flex min-h-[150px] w-full p-3">
    <div class="relative flex flex-1 flex-col border border-dashed bg-gray-50 p-3">
      <p class="m-auto text-center text-sm leading-none" x-text="name || 'KLIK DISINI ATAU SERET VIDIO KESINI UNTUK MEMBUAT EPISODE BARU'"></p>
      <p class="text-center text-sm" x-show="uploading" x-text="message"></p>
      <input type="file" accept="video/*" wire:model="video" x-on:change="onChange" class="absolute top-0 left-0 z-10 block h-full w-full opacity-0" />
    </div>
  </div>
</form>
